...
| 协议 | 当前探测方式 | 识别所用的主要证据 |
|---|---|---|
| SSH | 连接后先等待服务端版本行；若首次读取超时，再发送标准 SSH 识别行并读取一次。 | 版本行中的软件名称、版本、协议版本和附加系统文字。 |
| FTP | 先读取服务端欢迎语；欢迎语为空时，依次使用 `HELP`、`SYST` 和 `FEAT` 补充响应。 | 欢迎语、系统类型响应和 `FEAT`（服务器声明支持的 FTP 功能）中的软件名称、版本和能力。 |
| Telnet | 先进行空探测，即连接后不先发送应用层内容，只读取欢迎文字和 IAC 协商；收到 IAC 时按协商要求回复。若文字不足，再发送换行和 `admin` 触发附加提示。 | 可读欢迎文字、登录提示、IAC（Telnet 选项协商控制字节）特征和原始字节摘要。 |
| Redis | 主动发送 `PING`，成功后发送 `INFO server`。 | RESP 响应中的服务名称、版本、运行模式、操作系统和服务器字段。 |
| MySQL | 被动等待服务端在连接后主动发送初始握手包。 | 握手包中的协议版本、服务端版本、认证插件、能力标志和错误字段。 |
| PostgreSQL | 主动发送 `SSLRequest`；根据响应建立 TLS 或明文连接后，发送最小 StartupMessage。 | SSL 响应、认证消息、参数状态、错误响应和 SQLSTATE。 |
